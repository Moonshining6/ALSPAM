...
async def spam(event: events):
    if event.sender_id in SUDO_USERS:
        VAPOREON = event.text.split(" ", 2)
        mk = await event.get_reply_message()

        try:
            if len(VAPOREON) == 3:
                message = VAPOREON[2]
                for _ in range(int(VAPOREON[1])):
...
async def pspam(event):
    if event.sender_id in SUDO_USERS:
        if event.chat_id in GROUP:
            await event.reply("» ꜱᴏʀʀʏ, ᴛʜɪꜱ ɪꜱ ™°‌ 🫧 ☆𝐕ᴀᴘᴏʀᴇᴏɴ☆ ᴘʀᴏᴛᴇᴄᴛᴇᴅ ɢʀᴏᴜᴘ.")
        else:
            try:
...
async def hang(e):
    if e.sender_id in SUDO_USERS:
        if e.chat_id in GROUP:
            await e.reply("» ꜱᴏʀʀʏ, ᴛʜɪꜱ ɪꜱ ™°‌ 🫧 ☆𝐕ᴀᴘᴏʀᴇᴏɴ☆ ᴘʀᴏᴛᴇᴄᴛᴇᴅ ɢʀᴏᴜᴘ.")
        else:
            try:
