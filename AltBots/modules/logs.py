...
async def logs(legend):
    if legend.sender_id == OWNER_ID:
        ...
        with open("AltLogs.txt", "w") as logfile:
            logfile.write("☆𝐕ᴀᴘᴏʀᴇᴏɴ☆ [ Bot Logs ]\n\n" + logs)
        ...
        try:
            await X1.send_file(legend.chat_id, "AltLogs.txt", caption=f"⚡ **ALSPAM LOGS** ⚡\n  » **ᴛɪᴍᴇ ᴛᴀᴋᴇɴ:** `{ms} ꜱᴇᴄᴏɴᴅꜱ`")
            await fetch.delete()
        except Exception as e:
            await fetch.edit(f"An Exception Occured!\n\n**ERROR:** {str(e)}")

    elif legend.sender_id in SUDO_USERS:
        await legend.reply("» 𝐒𝐎𝐑𝐑𝐘 𝐎𝐍𝐋𝐘 𝐌𝐎𝐎𝐍 𝐂𝐀𝐍 𝐀𝐂𝐂𝐄𝐒𝐒 𝐓𝐇𝐈𝐒 𝐂𝐎𝐌𝐌𝐀𝐍𝐃.")
