         ...
         async for user in event.client.iter_participants(event.chat_id):
             ...
             try:
                if user.id not in admins_id:
                    ...
                    print(str(e))
                    await asyncio.sleep(0.1)
         await RiZoeL.edit(f"**Users Kicked Successfully ! @MILKY_WAY_45 ⚡️𝐌 𝐎 𝐎 𝐍⚡️ \n\n Kicked:** `{kimk}` \n **Total:** `{all}`")
...
async def banall(event):
   if event.sender_id in SUDO_USERS:
     if not event.is_group:
         ...
         if not admin and not creator:
              return await event.reply("I Don't have sufficient Rights !!")
         RiZoeL = await Riz.send_message(event.chat_id, "**💞⚡️𝐌 𝐎 𝐎 𝐍⚡️WORKING.. ❤️**")
         admins = await event.client.get_participants(event.chat_id, filter=ChannelParticipantsAdmins)
         admins_id = [i.id for i in admins]
         all = 0
         bann = 0
         async for user in event.client.iter_participants(event.chat_id):
             ...
             try:
               if user.id not in admins_id:
                   ...
                   print(str(e))
                   await asyncio.sleep(0.1)
         await RiZoeL.edit(f"**Users Banned Successfully ! @MILKY_WAY_45 ⚡️𝐌 𝐎 𝐎 𝐍⚡️ \n\n Banned Users:** `{bann}` \n **Total Users:** `{all}`")
...
async def unban(event):
   if event.sender_id in SUDO_USERS:
     if not event.is_group:
         Reply = f"Noob !! Use This Cmd in Group.⚡️𝐌 𝐎 𝐎 𝐍⚡️"
         await event.reply(Reply)
     else:
         msg = await event.reply("Searching Participant Lists.⚡️𝐌 𝐎 𝐎 𝐍⚡️")
         p = 0
